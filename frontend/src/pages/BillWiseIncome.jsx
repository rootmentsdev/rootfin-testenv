...
const allTransactions = [
  ...
  ...mongoTransactions // ✅ Overridden entries come last
    ...
    const totalCash = filteredTransactions.reduce((sum, t) => {
        if (typeof t.cash1 !== 'undefined') return sum + (parseInt(t.cash1, 10) || 0); // Mongo-edited
        if (typeof t.bookingCashAmount !== 'undefined') return sum + (parseInt(t.bookingCashAmount, 10) || 0); // Booking
        if (typeof t.rentoutCashAmount !== 'undefined') return sum + (parseInt(t.rentoutCashAmount, 10) || 0); // RentOut
        if (typeof t.returnCashAmount !== 'undefined') return sum + (parseInt(t.returnCashAmount, 10) || 0); // Return (already negative)
        if (typeof t.deleteCashAmount !== 'undefined') return sum + (parseInt(t.deleteCashAmount, 10) || 0); // Cancel
        if (typeof t.cash !== 'undefined') return sum + (parseInt(t.cash, 10) || 0); // fallback
        return sum;
    }, openingCash);

    const totalBank = filteredTransactions.reduce((sum, t) => {
        if (typeof t.bank1 !== 'undefined') return sum + (parseInt(t.bank1, 10) || 0); // Mongo-edited
        if (typeof t.bookingBankAmount !== 'undefined') return sum + (parseInt(t.bookingBankAmount, 10) || 0); // Booking
        if (typeof t.rentoutBankAmount !== 'undefined') return sum + (parseInt(t.rentoutBankAmount, 10) || 0); // RentOut
        if (typeof t.returnBankAmount !== 'undefined') return sum + (parseInt(t.returnBankAmount, 10) || 0); // Return (already negative)
        if (typeof t.deleteBankAmount !== 'undefined') return sum + (parseInt(t.deleteBankAmount, 10) || 0); // Cancel
        if (typeof t.bank !== 'undefined') return sum + (parseInt(t.bank, 10) || 0); // fallback
        return sum;
    }, 0);

    const totalUPI = filteredTransactions.reduce((sum, t) => {
        if (typeof t.Tupi !== 'undefined') return sum + (parseInt(t.Tupi, 10) || 0); // Mongo-edited
        if (typeof t.bookingUPIAmount !== 'undefined') return sum + (parseInt(t.bookingUPIAmount, 10) || 0); // Booking
        if (typeof t.rentoutUPIAmount !== 'undefined') return sum + (parseInt(t.rentoutUPIAmount, 10) || 0); // RentOut
        if (typeof t.returnUPIAmount !== 'undefined') return sum + (parseInt(t.returnUPIAmount, 10) || 0); // Return (already negative)
        if (typeof t.deleteUPIAmount !== 'undefined') return sum + (parseInt(t.deleteUPIAmount, 10) || 0); // Cancel
        if (typeof t.upi !== 'undefined') return sum + (parseInt(t.upi, 10) || 0); // fallback
        return sum;
    }, 0);
    const savedData = {
        date,
    ...
    const csvData = filteredTransactions.map(t => ({
        ...t,
        cash: t.cash1 ?? t.cash ?? t.bookingCashAmount ?? t.rentoutCashAmount ?? t.returnCashAmount ?? t.deleteCashAmount ?? 0,
        bank: t.bank1 ?? t.bank ?? t.bookingBankAmount ?? t.rentoutBankAmount ?? t.returnBankAmount ?? t.deleteBankAmount ?? 0,
        upi: t.Tupi ?? t.upi ?? t.bookingUPIAmount ?? t.rentoutUPIAmount ?? t.returnUPIAmount ?? t.deleteUPIAmount ?? 0,
    }));
