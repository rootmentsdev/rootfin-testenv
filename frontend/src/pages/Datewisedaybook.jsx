...
const buildOpeningUrl = ({ locCode }, date) => {
  ...
  const prevStr =
    new Date(date) < new Date('2025-01-01')
      ? '2025-01-01'
...
const Datewisedaybook = () => {
  ...
  /* ── block back-button after print (unchanged logic) ──────────── */
  useEffect(() => {
  ...
  const handleFetch = useCallback(async () => {
    ...
    /* ── mappers (keep original calculations) ─────────────────── */
    ...
    try {
      const res = await fetch(
        `${baseUrl.baseUrl}api/tws/getEditedTransactions?fromDate=${fromDate}&toDate=${toDate}&locCode=${currentUser.locCode}`
      );
