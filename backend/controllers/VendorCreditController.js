...
export const createVendorCredit = async (req, res) => {
  try {
    ...
    // vendorId is now a vendor ID reference (optional field)
    ...
    if (creditData.status === "open") {
      ...
      if (hasItemsWithQuantity(creditData.items)) {
        ...
        if (stockReductionErrors.length > 0) {
          ...
          await VendorCredit.findByIdAndDelete(vendorCredit._id);
  ...
  try {
    ...
    if (creditData.creditNoteNumber) {
      const existingCreditByNumber = await VendorCredit.findOne({ 
        ...
        _id: { $ne: id } // Exclude current credit
      });
    ...
    // Update the credit
    const vendorCredit = await VendorCredit.findByIdAndUpdate(id, creditData, { new: true });
    
    if (!vendorCredit) {
      return res.status(404).json({ message: "Vendor credit not found" });
    }
    
    console.log(`Vendor credit ${vendorCredit.creditNoteNumber} updated in MongoDB with ID: ${vendorCredit._id}`);
    res.status(200).json(vendorCredit);
  } catch (error) {
...
export const deleteVendorCredit = async (req, res) => {
  try {
    ...
    const deletedCredit = await VendorCredit.findByIdAndDelete(id);
...
export const applyCreditToBill = async (req, res) => {
  try {
    ...
    const updatedCredit = await VendorCredit.findById(creditId);
    res.status(200).json({
      message: "Credit applied to bill successfully",
      vendorCredit: updatedCredit,
      bill: bill,
    });
  } catch (error) {
