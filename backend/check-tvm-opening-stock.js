// Check TVM (Trivandrum) opening stock - compare image data with backend database
...
const matchesTrivandrum = (warehouseName) => {
...
// Check TVM opening stock discrepancy
const checkTVMOpeningStock = async () => {
  console.log('\n=== CHECKING TVM OPENING STOCK (Image vs Database) ===\n');
  
  // Data from your image (TVM 21-01-2026)
  const imageData = {
  ...
  try {
    // Get all items with Trivandrum opening stock from database
    const standaloneItems = await ShoeItem.find({
      isActive: { $ne: false },
      warehouseStocks: {
        $elemMatch: {
          warehouse: { $regex: /trivandrum|trivandum|grooms.*trivandrum/i },
          openingStock: { $gt: 0 }
        }
      }
    });
    ...
    for (const item of standaloneItems) {
      for (const ws of item.warehouseStocks) {
        if (matchesTrivandrum(ws.warehouse) && ws.openingStock > 0) {
    ...
    console.error('❌ Error checking TVM stock:', error);
  }
};
...
const main = async () => {
  await connectDB();
  await checkTVMOpeningStock();
  
  console.log('\n=== TVM STOCK CHECK COMPLETED ===');
